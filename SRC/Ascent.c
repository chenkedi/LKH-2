...
GainType Ascent()
{
  ...
  Start:
    ...
    if (CandidateSetType == DELAUNAY)
        ...
        AddTourCandidates(); // 若MaxCandidates指定的候选集数量为0，则通过事先给定的TourFile，将候选集加入

    /* Compute the cost of a minimum 1-tree */
    // 计算Minimum1TreeCost时会先求最小生成树，此时pi值为0，w为未进行次梯度优化的初始cost
    // 参数意义为是否是稀疏图
    ...
    if (!Subgradient || !Norm)
        return W;
    // Optimum是人为输入的已知最优解的权值, 不给定的话就是MINUS_INFINITY；若已知最优解给定了，且经过Precision放大后减去1-tree的初始cost仍大于0，则将这个差值作为MaxAlpha
    ...
    if (MaxCandidates > 0) {
        ...
        else { // 若等于DELAUNAY这种candidate类型，则需要额外在进行一次Minimum1Tree的生成
    ...
    BestNorm = Norm;
    InitialPhase = 1;
    /* Perform subgradient optimization with decreasing period length
