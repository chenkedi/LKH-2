...
void ReadProblem()
{
    ...
    Distance = 0; // Distance，C，c 都是由CostFunction函数指针类型定义的函数指针
    C = 0;
    c = 0;
    ...
    if (CostMatrix == 0 && Dimension <= MaxMatrixDimension && 
        ...
        Node *Ni, *Nj;
        assert(CostMatrix = // 这里costMatrix 是用一维数组来存储2维矩阵下三角的指针。由于节点到自己的距离恒为1，且if条件已经将ASTP等非对称情况排除，所以最后需要的数组大小就为D*D-1 /2
               (int *) calloc((size_t) Dimension * (Dimension - 1) / 2,
                              sizeof(int)));
        Ni = FirstNode->Suc;
        do {
            Ni->C = // C为CostMatrix中该Node与其他Node下三角矩阵的首元素 -1 的地址，表示一行，为何减一原因不明
                &CostMatrix[(size_t) (Ni->Id - 1) * (Ni->Id - 2) / 2] - 1;
            if (ProblemType != HPP || Ni->Id < Dimension) // 仅在问题为HPP类型，且到达计算下三角矩阵的最后一行时，该下三角矩阵的最后一行才设置为0
                for (Nj = FirstNode; Nj != Ni; Nj = Nj->Suc) // 由于Ni->C[Nj->Id]中Id是从1开始计数，而C数组从0开始计数，所以上面的首元素地址要-1
                    Ni->C[Nj->Id] = Fixed(Ni, Nj) ? 0 : Distance(Ni, Nj); // 计算下三角矩阵的值，Fixed的意义暂不明确
            else
                for (Nj = FirstNode; Nj != Ni; Nj = Nj->Suc) // 参见if处的讲解
                    Ni->C[Nj->Id] = 0;
        }
        while ((Ni = Ni->Suc) != FirstNode);
        WeightType = EXPLICIT; // cost下三角矩阵已经计算完毕，边的权重类型变为可以直接从数组读取
        c = 0; // 由于已经计算出来所有的距离，所以距离函数指针c重新置为0
    }
    if (Precision > 1 && (WeightType == EXPLICIT || ProblemType == ATSP)) { // 检测precision对当前的距离是否缩放是否不会失真，而不会留下小数点
    ...
    C = WeightType == EXPLICIT ? C_EXPLICIT : C_FUNCTION; // 根据上面计算出的WeightType来决定Cost计算函数和D矩阵计算函数的类型
    D = WeightType == EXPLICIT ? D_EXPLICIT : D_FUNCTION;
    if (SubsequentMoveType == 0) // 子序列MoveType表示在第一次Move后使用的Move类型，当大于等于2时表示使用自适应的K Move
        SubsequentMoveType = MoveType;
    K = MoveType >= SubsequentMoveType // 优先选择MoveTyp的值，除非subSeqentMoveType不等于0
        || !SubsequentPatching ? MoveType : SubsequentMoveType; // subsequentPatching是什么意思，何谓在子序列中使用patching
    if (PatchingC > K) // PatchingC表示允许的可以被Patched的不相交环的最大数量
        PatchingC = K; // 可以被Patched的不相交环的最大数量不超过K
    if (PatchingA > 1 && PatchingA >= PatchingC) // PatchingA 表示可以用来patch不相交环的额外环的数量
        PatchingA = PatchingC > 2 ? PatchingC - 1 : 1; // 当alternative的环大于最大不相交环数量，且最大不相交环大于2时，A要自适应为patchingC减一
    if (NonsequentialMoveType == -1 ||
        NonsequentialMoveType > K + PatchingC + PatchingA - 1) // 这一段对NonsequentialMoveType的自适应没搞懂？？
        NonsequentialMoveType = K + PatchingC + PatchingA - 1;
    if (PatchingC >= 1 && NonsequentialMoveType >= 4) { // 如果可被patched的环大于1，且需要NonsequencialMove，则使用BestKOptMove函数
        BestMove = BestSubsequentMove = BestKOptMove;
        if (!SubsequentPatching && SubsequentMoveType <= 5) { // 子序列patching要为0，并且子序列Move<=5,则定义一个move函数数组
            MoveFunction BestOptMove[] =
                { 0, 0, Best2OptMove, Best3OptMove,
                Best4OptMove, Best5OptMove
            };
            BestSubsequentMove = BestOptMove[SubsequentMoveType]; // SubseqentMoveType是，就用通过数组索引使用BestKOptMove
        }
    } else {
        ...
        BestSubsequentMove = SubsequentMoveType <= 5 ?
            BestOptMove[SubsequentMoveType] : BestKOptMove; // 这里的if else 主要是要确定 BestMove和BestSubsequentMove两个函数指针，是使用自适应的KOptMove还是固定数字的move函数
    }
    if (ProblemType == HCP || ProblemType == HPP)
        MaxCandidates = 0; // 为何汉密尔顿环和回路问题MaxCandidates为0？？？
    if (TraceLevel >= 1) {
        printff("done\n");
        PrintParameters();
    } else
    ...
    fclose(ProblemFile);
    if (InitialTourFileName) // 后续为读取各种初始化就给定的tour和subproblemTour的文件
...
static void CheckSpecificationPart()
{
    ...
    if (WeightType == EXPLICIT && WeightFormat == FUNCTION)// 当边的weight直接给出，weightFormat不能为函数
    ...
    if (WeightType != EXPLICIT
        ...
        eprintf("Conflicting EDGE_WEIGHT_TYPE and EDGE_WEIGHT_FORMAT");
    if (ProblemType == ATSP && WeightType != EXPLICIT && WeightType != -1)// 当问题类型为ASTP时，weightType必须为显示给定或没有指定
        eprintf("Conflicting TYPE and EDGE_WEIGHT_TYPE");
    if (ProblemType == ATSP && WeightFormat != FULL_MATRIX)// 问题类型为ASTP时，显示给出的weight矩阵必须为Full matrix
...
static void CreateNodes()
{
    ...
    for (i = 1; i <= Dimension; i++, Prev = N) {
        ...
        if (i == 1)
            ...
            Link(Prev, N); // 宏替换，Prev 和 N两个节点进行双向连接
        N->Id = i;
        if (MergeTourFiles >= 1) // 若存在MergeTourFiles，则需要为每个node的MergeSuc字段申请MergeTourFiles个Node * 大小的内存
            assert(N->MergeSuc =
...
{
    // strtok 第一次调用以后，第一个参数设置为0，可以继续往后分割字符串
...
static void Read_NODE_COORD_SECTION()
{
    ...
    N = FirstNode;
    do // 检查是否有所有的Node在Id合法的情况下，V值为0（即没该id的Node的坐标再ProblemFile中没有给出）
